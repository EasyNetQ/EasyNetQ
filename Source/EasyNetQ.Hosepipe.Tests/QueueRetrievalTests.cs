...
public class QueueRetrievalTests
{
    ...
    public async Task TryGetMessagesFromQueueAsync()
    {
        ...
        await foreach (var message in queueRetrieval.GetMessagesFromQueueAsync(parameters, cancellationToken: default))
        {
    ...
    public async Task ConsumeMessages()
    {
        ...
        var bus = provider.GetRequiredService<IBus>();
        await using var subscription = await bus.PubSub.SubscribeAsync<TestMessage>("hosepipe", message => Console.WriteLine(message.Text), cancellationToken: default);
