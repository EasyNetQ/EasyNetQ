...
public class ErrorRetryTests
{
    ...
    public ErrorRetryTests()
    {
        ...
        errorRetry = new ErrorRetry(new Serialization.NewtonsoftJson.NewtonsoftJsonSerializer(), new DefaultErrorMessageSerializer());
    }
    ...
    public async Task Should_republish_all_error_messages_in_the_given_directory()
    {
        ...
        var rawErrorMessages = new MessageReader()
           .ReadMessagesAsync(parameters, conventions.ErrorQueueNamingConvention(default), cancellationToken: default);

        await errorRetry.RetryErrorsAsync(rawErrorMessages, parameters, cancellationToken: default);
    }
}
