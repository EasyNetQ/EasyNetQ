...
public class When_using_default_consume_error_strategy
{
    ...
    public async Task Should_Ack_canceled_message()
    {
        ...
        var cancelAckStrategy = await mockBuilder.ConsumeErrorStrategy.HandleCancelledAsync(consumerExecutionContext, TestContext.Current.CancellationToken);
    ...
    public async Task Should_Ack_failed_message_When_publish_confirms_off()
    {
        ...
        var errorAckStrategy = await mockBuilder.ConsumeErrorStrategy.HandleErrorAsync(consumerExecutionContext, new Exception(), TestContext.Current.CancellationToken);
