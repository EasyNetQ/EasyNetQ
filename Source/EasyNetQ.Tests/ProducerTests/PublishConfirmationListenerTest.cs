...
public class PublishConfirmationListenerTest : IDisposable
{
    ...
    {
        channel.GetNextPublishSequenceNumberAsync(CancellationToken.None).Returns(DeliveryTag);
        var confirmation = await publishConfirmationListener.CreatePendingConfirmationAsync(channel, cancellationToken: CancellationToken.None);
    ...
    {
        channel.GetNextPublishSequenceNumberAsync(CancellationToken.None).Returns(DeliveryTag - 1, DeliveryTag);
        var confirmation1 = await publishConfirmationListener.CreatePendingConfirmationAsync(channel, cancellationToken: CancellationToken.None);
        var confirmation2 = await publishConfirmationListener.CreatePendingConfirmationAsync(channel, cancellationToken: CancellationToken.None);
    ...
    {
        channel.GetNextPublishSequenceNumberAsync(CancellationToken.None).Returns(DeliveryTag);
        var confirmation1 = await publishConfirmationListener.CreatePendingConfirmationAsync(channel, CancellationToken.None);
