...
public class PublishConfirmationListenerTest : IDisposable
{
    ...
    {
        channel.GetNextPublishSequenceNumberAsync(cancellationToken: default).Returns(DeliveryTag);
        var confirmation = await publishConfirmationListener.CreatePendingConfirmationAsync(channel, cancellationToken: default);
    ...
    {
        channel.GetNextPublishSequenceNumberAsync(cancellationToken: default).Returns(DeliveryTag - 1, DeliveryTag);
        var confirmation1 = await publishConfirmationListener.CreatePendingConfirmationAsync(channel, cancellationToken: default);
        var confirmation2 = await publishConfirmationListener.CreatePendingConfirmationAsync(channel, cancellationToken: default);
