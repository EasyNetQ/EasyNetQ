...
public class When_an_action_is_invoked_that_throws_using_multi_channel : IAsyncLifetime
{
    ...
    public When_an_action_is_invoked_that_throws_using_multi_channel()
    {
        ...
        using var producerConnection = Substitute.For<IProducerConnection>();
...
#pragma warning restore IDISP004
        channel.InvokeChannelActionAsync<int, FuncBasedPersistentChannelAction<int>>(default)
            .ReturnsForAnyArgs(x => ((FuncBasedPersistentChannelAction<int>)x[0]).InvokeAsync(model, CancellationToken.None).Result);
    ...
    public async Task Should_raise_the_exception_on_the_calling_thread()
    {
        await Assert.ThrowsAsync<CrazyTestOnlyException>(
            () => dispatcher.InvokeAsync<int>(_ => throw new CrazyTestOnlyException(), PersistentChannelDispatchOptions.ProducerTopology, cancellationToken: CancellationToken.None).AsTask()
        );
    }
    ...
    public async Task Should_call_action_when_previous_threw_an_exception()
    {
        await Assert.ThrowsAsync<Exception>(
            () => dispatcher.InvokeAsync<int>(_ => throw new Exception(), PersistentChannelDispatchOptions.ProducerTopology, cancellationToken: CancellationToken.None).AsTask()
        );

        var result = await dispatcher.InvokeAsync(_ => Task.FromResult(42), PersistentChannelDispatchOptions.ProducerTopology, cancellationToken: CancellationToken.None);
        result.Should().Be(42);
    }
