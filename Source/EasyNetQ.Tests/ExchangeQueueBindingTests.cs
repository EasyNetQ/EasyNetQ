...
public class When_a_queue_is_deleted_with_name : IAsyncLifetime
{
    ...
    public async Task Should_delete_the_queue()
    {
        await mockBuilder.Channels[0].Received().QueueDeleteAsync("my_queue", false, false, cancellationToken: default);
    }
}
...
public class When_an_exchange_is_declared_passively : IAsyncLifetime
{
    ...
    public async Task Should_passively_declare_exchange()
    {
        ...
        await mockBuilder.Channels[0].Received().ExchangeDeclarePassiveAsync(Arg.Is("my_exchange"), cancellationToken: default);
    }
}
...
public class When_an_exchange_is_deleted : IAsyncLifetime
{
    ...
    public async Task Should_delete_the_queue()
    {
        await mockBuilder.Channels[0].Received().ExchangeDeleteAsync("my_exchange", false, cancellationToken: default);
    }
}
...
public class When_a_queue_is_unbound_from_an_exchange : IAsyncLifetime
{
    ...
    {
        await mockBuilder.Channels[0].Received().QueueUnbindAsync("my_queue", "my_exchange", "my_routing_key", null, cancellationToken: default);
    }
}
