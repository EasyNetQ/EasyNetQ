...
public class VersionedExchangeDeclareStrategyTests
{
    ...
    public async Task When_declaring_exchanges_for_unversioned_message_one_exchange_created()
    {
        ...
        var advancedBus = Substitute.For<IAdvancedBus>();
        advancedBus.ExchangeDeclareAsync(Arg.Any<string>(), cancellationToken: default)
        ...
        advancedBus.ExchangeBindAsync(Arg.Any<string>(), Arg.Any<string>(), Arg.Is("#"), Arg.Any<IDictionary<string, object>>(), cancellationToken: default)
            .Returns(mi =>
            {
        ...
        await publishExchangeStrategy.DeclareExchangeAsync(typeof(MyMessage), ExchangeType.Topic, cancellationToken: default);
    ...
    public async Task When_declaring_exchanges_for_versioned_message_exchange_per_version_created_and_bound_to_superceding_version()
    {
        ...
        await publishExchangeStrategy.DeclareExchangeAsync(typeof(MyMessageV2), ExchangeType.Topic, cancellationToken: default);
