...
namespace EasyNetQ
{
    public class RabbitAdvancedBus : IAdvancedBus
    {
        ...
        public IDisposable Respond(
            ...
            string queue, 
            string topic,
            Func<SerializedMessage, MessageReceivedInfo, Task<SerializedMessage>> handleRequest)
        {
            return advancedServerRpc.Respond(requestExchangeName, queue, topic, handleRequest);
        }
        ...
        {
            return advancedServerRpc.Respond(requestExchangeName, queue, topic, (message, info) => handleRequest(message));
        }
